...
> Note. It is strongly discourage to have entry and exit bottlenecks with finite flows for all edges
> of the road network as it will slow down the simulator significantly.
...
the bottleneck.
Therefore, a piecewise-linear time function is built with the following points:

- A point is added for the start of the recording period with zero waiting time.
- Each time a vehicle exits the bottleneck, a point is added with the vehicle's entry and waiting
  time.
- Each time the queue gets empty (i.e., the last vehicle in the queue exits the bottleneck through a
  `BottleneckEvent`), a point is added with the exit time of the last vehicle as entry time and zero
  waiting time.

> Note. Observe that, if a single vehicle crosses the bottleneck during the recording period, at
> time \\( t \\). Then, the recorded waiting time will be of zero during the whole period, even
> though a vehicle arriving during at the bottleneck during the time interval \\( [t, t + v^{PCE} /
> s^o] \\) would face some waiting time. This is done on purpose. It means that if the same vehicle
> were to arrive at the bottleneck at about the same time on the next day, it would not anticipate
> some waiting time just because it generated some waiting time itself the day before.
>
