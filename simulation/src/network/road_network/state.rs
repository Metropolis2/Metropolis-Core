    ...
    fn enters(
        ...
        if let Some(opening_time) = self.next_opening {
            if opening_time <= current_time {
                ...
                self.next_opening = None;
                let next_opening = self.get_next_opening(vehicle, opening_time);
...
impl<'a, T: TTFNum + 'static> RoadEdgeState<'a, T> {
    ...
    ///
    /// Return a [BottleneckStatus] that represents the state of the Bottleneck (See
    /// [Bottleneck::enters]).
    pub fn enters_bottleneck(
        ...
        if let Some(bottleneck) = &mut self.bottleneck {
            bottleneck.enters(event, vehicle, current_time, self.edge_index)
        } else {
